...
export default function RecipientInput({ value, onChange }: Props) {
  ...
  // ── 재귀 트리 노드 렌더링 ──────────────────────────────────────
  function renderGroupNode(g: GroupRow): React.ReactNode {
      ...
      <div key={g.id} className={styles.treeNodeWrap}>
        ...
        >
          <span className={`${styles.treeToggle} ${isExpanded ? styles.treeToggleOpen : ""}`}>
            {hasChildren || !hasMemberData ? (isExpanded ? "▼" : "▶") : (isExpanded ? "▼" : "▶")}
          </span>
        ...
        {isExpanded && (
          <div className={styles.treeChildren}>
            {/* 자식 그룹 먼저 */}
            {hasChildren && g.children!.map(child => renderGroupNode(child))}

            {/* 직접 멤버 목록 */}
            <div className={styles.treeMemberSection} style={{ marginLeft: indent + 32 }}>
              {isLoading ? (
